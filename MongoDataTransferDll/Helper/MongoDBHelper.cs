...
namespace OSSP.BLIService.DataTransferDll
{
    public class MongoDBHelper
    {
        ...
        public void InsertBatch(string tableName, List<BsonDocument> documents)
        {
            MongoCollection collection = mongoDatabase.GetCollection(tableName);
            collection.InsertBatch(documents, SafeMode.True);
        }

        public void Update(string tableName,IMongoQuery query,IMongoUpdate update)
        {
            var collection = mongoDatabase.GetCollection(tableName);
            collection.Update(query, update,UpdateFlags.Multi);
        }

        public bool QueryOne(string tableName, IMongoQuery query, ref UpdateDocument oldDocument)
        {
            MongoCollection<UpdateDocument> collection =
                mongoDatabase.GetCollection<UpdateDocument>(tableName);


            UpdateDocument bsonDocument = collection.FindOne(query);
